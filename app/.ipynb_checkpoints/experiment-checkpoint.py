...
if __name__ == '__main__':
    ITERS = 50
    log.info('-' * 30)
    log.info('THROMBOSE Detection with Machine Learn')
    log.info('-' * 30)

    for test_size in [0.1, 0.15, 0.2]:
        process_A(test_size, NUM_ITER=ITERS, folder='results-smote')
        process_B(test_size, NUM_ITER=ITERS, folder='results-smote')
        process_C(test_size, NUM_ITER=ITERS, folder='results-smote')
        process_D(test_size, NUM_ITER=ITERS, folder='results-smote')
