...
import pandas as pd
import random
from ialovecoffe.data import read_dataset_D as read_dataset

"""
def read_data():
    dataframe_reader = pd.read_csv("data/thrombosis_non_thrombosis_v2.csv", sep="\t")
    return dataframe_reader
""" 
...
def run_pipe():
    ...
    for i in np.arange(n_iter):
        ...
        datafds = data_splitting(x, y, df,  smote=smote, test_size=5)
        NF = len(datafds)
        ...
        for j in range(NF): 
            ...
            log.info(f'LocalOutlierFactor..: {f1} acc class 1: {acc1} acc class 2: {acc2}')
            
            save_model(model, filename=f"LOF_iter-{i}_fold-{j}_f1-{f1}.pkl", 
                              folder='saved_models/D/')
    ...
    df.to_csv(f'{folder}D-RS-ALL-{n_iter}_Fds-{NF}_norm-{norm}_stand-{stand}_smote-{smote}_{dt_string}.csv', index=False)
