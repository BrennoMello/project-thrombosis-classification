...
#' @param label.index - the reference to the data class
...
cv.bin.strat.class<-function(dataset, label.index=1, seed=123456, cv=10){
  set.seed(seed)

  cv.folds<-list()

  balance<-table(dataset[, label.index])  
  ...
  for(cv.i in 1:(cv-1)){
    tmp<-which(dataset[actual.inst, label.index]==(names(balance)[1]))
    c1<-actual.inst[sample(tmp, n.inst.c0)]
    tmp<-which(dataset[actual.inst, label.index]==(names(balance)[2]))
