...
def runXGBoost(train_fold, test_fold, positive, negative, metric = 'auc', class_idx = np.nan):
    ...
    #print("Predictions ",myfit.predict(d_test))
    predictions = (myfit.predict(d_test))
...
def runSFC(dataset, nrepetitions = 1, class_idx = np.nan, 
