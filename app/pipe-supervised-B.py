...
from ialovecoffe.data import read_dataset_B
...
def run_pipe():
    ...
    df['type'] = df['type'].replace(['Type_I', 'Type_II'], [0, 1]).to_numpy()
    ...
    for i in np.arange(n_iter):
        ...
        datafds = data_splitting(x, y, df, smote=smote, test_size=4)
        NF = len(datafds)
    ...
    df.to_csv(f'{folder}B-RS-ALL-{n_iter}_Fds-{NF}_norm-{norm}_stand-{stand}_smote-{smote}_{dt_string}.csv', index=False)
