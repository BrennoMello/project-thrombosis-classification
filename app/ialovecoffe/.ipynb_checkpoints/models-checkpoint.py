...
def RSOneClassSVM(X_train, y_train, X_test, y_test, measure='f1_macro'):
    
    pipe = Pipeline([
    ("clf", OneClassSVM(max_iter = 100, kernel='poly'))
    ])

    params = {
        "clf__gamma": np.arange(0.01, 2, 0.1),
        "clf__coef0": np.arange(0, 2, 0.1),
        "clf__degree": np.arange(2, 5, 1)
    }

    model_cv = RandomizedSearchCV(pipe, params, n_iter=1000, scoring=measure, n_jobs = -1, random_state=1)
...
def RSIsolationForest(X_train, y_train, X_test, y_test):
