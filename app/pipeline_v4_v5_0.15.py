...
def run_experiment(x, y, iterations) -> pd.DataFrame:
    ...
    for i in tqdm(range(iterations)):
        ...
        x_train, y_train, x_test, y_test = create_balanced_folds(x, y, rs=i, percentage=0.15)
        print(len(y_train))
        print(len(y_test))
...
def process_A():
    ...
    df.to_csv('results/results_v4_v5_0.15/A_thrombosis_non_thrombosis_v5.csv', index=False)
...
def process_B():
    ...
    # run
    df = run_experiment(x, y, NUM_ITER)
    df.to_csv('results/results_v4_v5_0.15/B_type_I_PE_vs_Type_II_v4.csv', index=False)
...
def process_C():
    ...
    # run
    df = run_experiment(x, y, NUM_ITER)
    df.to_csv('results/results_v4_v5_0.15/C_type_I_vs_Type_II_no_PE_v4.csv', index=False)
...
def process_D():
    ...
    df = run_experiment(x, y, NUM_ITER)
    df.to_csv('results/results_v4_v5_0.15/D_type_I_vs_type_II_v4.csv', index=False)
